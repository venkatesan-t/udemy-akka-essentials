...
class MultithreadingRecap extends App {
  ...
  val cThread = new Thread(() => println("I'm running in parallel"))

  aThread.start()
  aThread.join()

  // problem with threads are they are unpredictable
  ...
  T1 -> this.amount = this.amount - 1000 // PREEMPTED by the OS
  ...
  val filteredFuture = future.filter(_ % 2 == 0) // when filter not matches, NoSuchElementException is thrown

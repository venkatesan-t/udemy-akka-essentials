...
class InterceptionLogsSpec extends TestKit(
  ActorSystem("InterceptionLogsSpec", ConfigFactory.load().getConfig("interceptingLogMessages")))
  with ImplicitSender
  ...
  class CheckoutActor extends Actor {
    ...
    def pendingPayment(item: String): Receive = {
      case PaymentAccepted =>
        fulfillmentManager ! DispathOrder(item)

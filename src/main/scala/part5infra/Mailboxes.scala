  ...
  class SupportTicketPriorityMailbox(settings: ActorSystem.Settings, config: Config)
    extends UnboundedPriorityMailbox(
      PriorityGenerator {
        ...
        case message: String if message.startsWith("[P2]") => 3
        case _ => 4
      })
...
//  supportTicketLogger ! "[P1] do this when you have the time"
  ...
  // after which time can I send another message and be prioritized accordingly? - cannot know or configured

  /**
   * Interesting case #2 - control-aware mailbox
   * we'll use UnboundedControlAwareMailbox
   */
  // step 1 - make important messages as control messages

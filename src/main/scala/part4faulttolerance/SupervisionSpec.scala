...
object SupervisionSpec {
  ...
  class FussyWordCounter extends Actor with ActorLogging {
    ...
    override def receive: Receive = {
      case Report =>
        sender() ! words
        log.info(s"words - $words - uuid - ${self.toString()}")
      case "" => throw new NullPointerException("sentence is empty")
      case sentence: String =>
        if (sentence.length > 20) throw new RuntimeException("sentence is too big")
        else if (!Character.isUpperCase(sentence.head)) throw new IllegalArgumentException("sentence must start with uppercase")
